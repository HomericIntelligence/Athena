...
| --- | --- |
| **GO** | Default profile; A (93–100); architecture aligned or evidenced intentional change; zero `required` findings; complete applicable source, scope, requirements, language, and validation coverage; host-selected local checks passing on the reviewed head; and exact-head provider evidence that all effective required pre-admission checks, rulesets, reviews, conversations, deployments, and integration policy gates are satisfied. The PR is OPEN, non-draft, conflict-free, `behind_count == 0`, and has no existing or planned required unresolved discussion. A merge queue is a route after pre-admission gates, not proof they passed. A GO eligible for auto-merge also needs an authenticated reviewer distinct from the PR author; no approval, bypass, or policy change is implied. |
