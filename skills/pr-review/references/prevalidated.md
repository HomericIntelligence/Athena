...
| Changed paths and lenses | Verify each declared range and digest. Bind changed paths to the null-character-safe (NUL-safe) author-intent manifest. Verify the current-target lens separately as diagnostic evidence. Verify both lens byte streams against their SHA-256 values. |
