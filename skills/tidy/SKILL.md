---
name: tidy
description: Safely tidy branches using required Hephaestus automation. Resolves HOMERIC_INTELLIGENCE_HEPHAESTUS_OWNER, a verified fork in the current repository owner, or HomericIntelligence/Hephaestus, and fails if ~/.agent_brain/automation cannot be prepared.
...
1. Confirm the repository and default branch. Stop if the working tree has uncommitted changes that
   would be affected.
2. Resolve Hephaestus using `HOMERIC_INTELLIGENCE_HEPHAESTUS_OWNER`, then a verified
   `<current-owner>/Hephaestus` fork, then `HomericIntelligence/Hephaestus`. Prepare it at
