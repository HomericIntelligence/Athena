...
| Issue-plan finalization | `--draft` is read-only. A verified finalized planning epoch may replace the resolved issue body once, then requires exact readback; it never mutates comments or other forge state. |
